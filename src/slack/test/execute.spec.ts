...
import * as slack from "../src/index";
...
describe("execute()", () => {
  ...
  it("should execute webhook", async() => {
    ...
    const { data } = await slack.execute({
      counter: 10,
  ...
  it("should fail executing webhook when Slack API rejects", async() => {
    pool.intercept({
      method: "POST",
      path: "/"
    }).reply(400, { message: "Unable to send webhook" });

    await assert.rejects(async() => await slack.execute({
      counter: 10,
      ruleConfig: kValidRuleConfig,
      webhookUrl: kDummyWebhoobURL,
      severity: "error",
